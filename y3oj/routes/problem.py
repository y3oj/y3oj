...
def route_problem_index():
    ...
    pagination = problems.paginate(page=int(request.args.get("page", 1)), per_page=int(config.per_page.problem))
...
def route_problem_submit(id):
    ...
    if form.validate_on_submit():
        submit_code(user=current_user, problem=g.problem, code=request.form['code'])
...
def route_problem_assets(id, path):
    return send_from_directory(os.path.join('..', 'data', 'problem', id, 'assets'), path)
