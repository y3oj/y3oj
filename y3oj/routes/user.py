...
def route_user_submission(id):
    ...
    submissions = query_user_submissions(g.user)
    pagination = submissions.paginate(page=int(request.args.get("page", 1)), per_page=int(config.per_page.submission))
    return render_template(
        'user/submission.html',
        user=g.user,
...
def route_user_settings(id):
    ...
    form = UserSettingsForm(summary=g.user.settings.get('summary'), description=g.user.settings.get('description'), background_image=g.user.settings.get('background_image'))

    if form.validate_on_submit():
        settings = g.user.settings

        if len(request.form['summary']) > config.user.limit.summary:
            return render_template('user/settings.html', user=g.user, form=form, snackbar_message=f'「一句话简介」超出长度限制（{config.user.limit.summary}）')

        if len(request.form['description']) > config.user.limit.description:
            return render_template('user/settings.html', user=g.user, form=form, snackbar_message=f'「关于我」超出长度限制（{config.user.limit.description}）')

        if len(request.form['background_image']) > config.database.limit.link:
            return render_template('user/settings.html', user=g.user, form=form, snackbar_message=f'「背景图片链接」不合法')
        ...
        if request.form['old_password'] and request.form['new_password'] and request.form['new_password_check']:
            ...
            if oldpass != g.user.password:
                return render_template('user/settings.html', form=form, user=g.user, snackbar_message='密码错误')

            if newpass != newpass_check:
                return render_template('user/settings.html', form=form, user=g.user, snackbar_message='两次输入的密码不一致')
