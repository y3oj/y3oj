...
def route_docs_index():
    return render_template(
        'docs/article.html',
        **render_markdown(readFile(path_join(docs_root, 'README.md')), anti_xss=False, frontmatter=True),
    )
...
def route_docs(path):
    ...
    basename, _ = os.path.splitext(os.path.basename(local_path))
    if path.startswith('.'):
        return abort(400)
    ...
    return render_template(
        'docs/article.html',
        title=basename,
        **render_markdown(readFile(local_path), anti_xss=False, frontmatter=True),
    )
